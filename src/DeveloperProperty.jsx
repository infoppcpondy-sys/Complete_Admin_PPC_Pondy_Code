  ...
  const fetchAllData = async () => {
    try {
      const [ developerRes] = await Promise.all([
        ...
        axios.get(`${process.env.REACT_APP_API_URL}/fetch-active-developer`),
      ]);
...
const filterList = (list) => {
  return list.filter(item => {
    const ppcId = String(item?.ppcId ?? '').toLowerCase();
    const phone = String(item?.phoneNumber ?? '');
    const postedBy = String(item?.postedBy ?? '').toLowerCase();
    const createdAt = item?.createdAt ? new Date(item.createdAt) : null;

    const matchPPCID = search ? ppcId.includes(search.toLowerCase()) : true;
    ...
    return matchPPCID && matchPhone && matchPostedBy && matchFromDate && matchEndDate;
  });
};
const handleReset = () => {
  setSearch('');
  ...
  const adminRole = reduxAdminRole || localStorage.getItem("adminRole");
   ...
   const fileName = "Developer Property"; // current file
  ...
  backgroundColor: '#fff' 
      ...
      >
        <div className="mb-3">
          <label htmlFor="searchInput" className="form-label fw-bold">PPC ID</label>
          <input
            type="text"
            id="searchInput"
...
export default DevoloperProperty;
