...
const [filters, setFilters] = useState({
      ...
      setMatchedData((prevData) =>
        prevData.map((item) =>
          item.buyerAssistanceCard._id === id
            ? { ...item, buyerAssistanceCard: { ...item.buyerAssistanceCard, isDeleted: true } }
            : item
        )
      );
    } catch (error) {
  ...
  const handleUndoDelete = async (id) => {
    ...
    try {
      ...
      setMatchedData((prevData) =>
        prevData.map((item) =>
          item.buyerAssistanceCard._id === id
            ? { ...item, buyerAssistanceCard: { ...item.buyerAssistanceCard, isDeleted: false } }
            : item
        )
      );
    } catch (error) {
  ...
  const fetchMatchedData = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${process.env.REACT_APP_API_URL}/fetch-all-matched-datas`);
      if (res.data.success) {
...
<div ref={tableRef}>        
    ...
    <thead className="sticky-top">
            <tr>
              <th><FaIdBadge className="me-1" /> Property ID</th>
              ...
              <th><FaIdBadge className="me-1" /> BA_ID</th>
              <th><FaUserTag className="me-1" /> BA_NAME</th>
              <th><FaPhone className="me-1" /> BA PHONE</th>
              <th><FaMapMarkerAlt className="me-1" /> BA AREA</th>
              <th><FaMapMarkerAlt className="me-1" /> BA CITY</th>
              <th>Status</th>
              <th>Action</th>
               <th>Views Details</th>
            </tr>
